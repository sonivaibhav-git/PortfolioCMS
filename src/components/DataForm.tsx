...
import type { PortfolioData, Project, Achievement, Tool, Technology } from '../types';
...
export default function DataForm() {
  ...
  const [activeSection, setActiveSection] = useState<'projects' | 'skills' | 'achievements'>('projects');
  ...
  // Skills form state
  const [tool, setTool] = useState<Tool>({ name: '', imageSrc: '' });
  const [technology, setTechnology] = useState<Technology>({ name: '', imageSrc: '' });
  const [techType, setTechType] = useState<'frontend' | 'backend' | 'database'>('frontend');
  const [language, setLanguage] = useState<{ name: string; imageSrc: string }>({ name: '', imageSrc: '' });
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="flex gap-4 mb-6">
            {(['projects', 'skills', 'achievements'] as const).map((section) => (
              <button
                key={section}
          ...
          {activeSection === 'skills' && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <select
                  value={techType}
                  onChange={(e) => setTechType(e.target.value as 'frontend' | 'backend' | 'database')}
